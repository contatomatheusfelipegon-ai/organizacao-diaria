...
const PRODUCT_CATS = [...new Set(PRODUCTS.map(p=>p.cat))].sort();
...
const INITIAL_SELLERS = ["Luiz Penha","Alessandro","Matheus Maurer","Diego Duque","Luciano","Nicolau Salomon"];
const INITIAL_RONALDO = [
...
// ─── Styles ──────────────────────────────────────────────────────────────────
const navBtn    = { background:"none",border:"none",fontSize:20,cursor:"pointer",color:"#7C3AED",padding:"4px 10px",borderRadius:6 };
const actionBtn = { border:"none",borderRadius:8,padding:"10px 14px",fontWeight:700,fontSize:13,cursor:"pointer" };
function inputStyle(t){ return { width:"100%",border:`1.5px solid ${t.border}`,borderRadius:8,padding:"10px 12px",fontSize:13,color:t.text,outline:"none",boxSizing:"border-box",background:t.inputBg,fontFamily:"inherit" }; }
function lblStyle(t){ return { display:"block",fontSize:11,color:t.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:.5,marginBottom:4 }; }
// Legacy fallbacks (light) kept for components that haven't been threaded with theme yet
const input = inputStyle(getTheme(false));
const lbl   = lblStyle(getTheme(false));
...
// ─── CnpjField extraído para fora do hook ────────────────────────────────────
...
function TabComercial({ selected, actions, setActions, theme }){
  ...
  // Usando hook com o componente extraído
...
function TabDegustacoes({ selected, actions, setActions, theme }){
  ...
  // Histórico: todas as degustações já registradas
